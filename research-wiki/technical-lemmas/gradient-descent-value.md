...
The final pre-push fetch found actual PR278 merge `41f04c0ecadc482ffee2db8ef977bceae2a984b1`. Incoming GaussianArcLaw and its source/reader/audit artifacts are preserved. The source registry union retains all63 common objects plus1 incoming and2 local objects (66 total). Under independent `smoothness_review` direction, the event ledger retains the exact common byte prefix, all8 local raw rows, then all10 incoming raw rows, without timestamp sorting or altering a historical row. The incoming original-owner PoissonQueryTail MERGED event therefore supersedes the branch-local stale reservation release. Independent historical preservation review authorized an appended GaussianArcLaw STABILIZING→VERIFIED release at its actual merged commit; SmoothGradientArcMoment remains EXPLORING under its existing owner. GDV is again the sole stabilization owner. See `upstream-review.json` beside the other review artifacts.

The former PASS9150 gate and UI checks remain evidence for fccbf94 only. The merged candidate must receive a fresh canonical aggregate gate and graph/site regeneration; unchanged proof/source/lesson/repair audits are reused because their exact bytes/bindings remain identical.

Merged candidate `21dfd69e6b27640c6859c16e617ee0a03ce99800` now passed the fresh canonical gate: PASS9152, including the real GDV and GaussianArcLaw root tests, at `2026-09-11T12:23:47.006138+00:00`. Independent merged-state review is preserved as `merged-review.json`. Refreshed site/graph checks passed: 622 modules,3764 declarations,412 compiled local leaves; 75 frontier cells,66 audits/3 repairs,52 publication source items. Both bounded GDV graph reports are byte-identical to the visually inspected reports; the proof/lesson/renderer are unchanged, so that desktop/mobile inspection is reused. A fresh browser reload confirms the new21dfd69 gate timestamp. Harness255/6-skip evidence remains applicable: incoming PR278 changed no tools/tests or protocol code. All merge-review conditions are satisfied; canonical delivery is next.
