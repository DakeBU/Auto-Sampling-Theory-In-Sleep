...
```bash
python3 tools/astis.py agent-note 20260525-183526-365387-ASTIS-SALD-001-cycle40 --role lower --message "..."
python3 tools/astis.py trial-log --task ASTIS-SALD-001 --role lower --kind handoff --status queued --artifact runs/20260525-183526-365387-ASTIS-SALD-001-cycle40 --notes "..."
```
...
## Middle-Prepared Lower Packet

Use the cycle-40 middle declarations rather than reopening the whole discrete
theorem:

- first target `sald.discrete_forward_kl.em_endpoint_laws` by instantiating
  `SALD.discreteForwardKlEmInterpolationLeftEndpointLawHandoff` and
  `SALD.discreteForwardKlEmInterpolationRightEndpointLawHandoff` with the
  concrete law/density notation for `hat rho_s`, `rho_k^eta`, and
  `rho_{k+1}^eta`;
- if that is blocked by missing law notation, refine exactly that interface and
  keep it an obligation;
- next target `sald.discrete_forward_kl.conditional_drift_density` for the
  regular conditional law and measurable/integrable `bar b_{k,s}`;
- only after those are stable, create a precise source-cited interface for
  `sald.discrete_forward_kl.em_conditional_fokker_planck` supplying the
  conditional-drift FP equation and Laplacian split consumed by
  `SALD.discreteForwardKlConditionalFpLaplacianSplitHandoff`.

Do not mark endpoint laws, conditional drift density, conditional-FP, KL
differentiation, LSI, DV, Gronwall, or `thm:forward-KL-discrete` formalized
unless the corresponding Lean proof builds locally.
