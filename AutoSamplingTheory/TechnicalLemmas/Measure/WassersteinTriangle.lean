...
/-!
# The L2 Minkowski core of the Wasserstein triangle inequality

For a joint law of `(X,Y,Z)`, the source proof is

`|X-Z| <= |X-Y| + |Y-Z|`

followed by Minkowski in `L2`.  This module isolates exactly that analytic
step before optimality and transport infima are introduced.
-/
...
/-- The extended-nonnegative edge lengths are measurable on a Borel normed space. -/
