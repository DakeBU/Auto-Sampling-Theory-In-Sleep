...
| [lean-stat-learning-theory](https://github.com/YuanheZ/lean-stat-learning-theory) and [arXiv:2602.02285](https://arxiv.org/abs/2602.02285) | Mathlib-based proof style for probability, concentration, entropy duality, log-Sobolev/Poincare facts, and discretization statements. | Because the upstream toolchain differs, ASTIS records it as audited port/reference memory rather than an immediate Lake dependency. | SDE/Sampling proofs can retrieve nearby measure-theory engineering without silently importing incompatible code. |
| [StatsMLlib](https://github.com/Lean-MoDS/StatsMLlib) and its [contributor guide](https://statsmllib.github.io/contribute.html) | Subject-owned module placement, reuse-first development, complete-proof policy, preserved source attribution, whole-library verification, and a four-stage discuss/develop/verify/submit contribution route. | Samplinglib adds exact textbook/paper source correspondence, independent local and route status, typed ASTIS handoffs, and reviewer-controlled admission to formal memory. The ASTIS page and repository guide are independently written for this workflow. | Contributors can enter through one visible process while reviewers retain sampling-specific semantic and certificate boundaries. |
