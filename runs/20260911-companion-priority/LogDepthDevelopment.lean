...
-- Quantitative bridge for the prescribed terminal stage; development only.
private theorem depth_indices {K B C : ℝ} (hK : 1 ≤ K) (hB : 0 < B) (hB1 : B ≤ 1)
    (hC : 8 ≤ C) :
...
private theorem remaining_contraction {K B C c : ℝ} (hK : 1 ≤ K) (hB : 0 < B) (hB1 : B ≤ 1)
...
private theorem denominator_log {d L : ℝ} (hd : 1 ≤ d) (hL : 2 ≤ L)
    (hld : Real.log d ≤ L) :
...
private theorem depth_upper {K d L γ C : ℝ} (hK : 1 ≤ K) (hd : 1 ≤ d)
...
private theorem source_log_domain {K q Δ : ℝ} {d : ℕ} (hK : 1 ≤ K)
...
private theorem actual_terminal {κ c r₀ B C : ℝ} {η : ℕ → ℝ}
...
example {κ c r₀ q Δ γ C : ℝ} {d : ℕ} {η : ℕ → ℝ}
    (hκ : 1 ≤ κ) (hc : 0 < c) (hc1 : c < 1/4) (hr₀ : 0 ≤ r₀)
