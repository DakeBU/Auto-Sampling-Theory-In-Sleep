...
This file is a clean port of the previously verified ASTIS branch
`simultaneous-f-divergence-spatial-gradients` (head `6745865c...`) onto the
current Samplinglib main line. It reuses the canonical gradient product/chain
...
theorem hasDerivAt_f_sub_id_mul_fPrime
  ...
  have hfun :
    ...
    rfl
  have hsub :
      HasDerivAt (f - id * fPrime)
        (fPrime r - (1 * fPrime r + r * fpp)) r :=
    hf.sub ((hasDerivAt_id r).mul hfPrime)
  have hcoeff :
      fPrime r - (1 * fPrime r + r * fpp) = -r * fpp := by
    ring
  rw [hfun]
  simpa [hcoeff] using hsub
