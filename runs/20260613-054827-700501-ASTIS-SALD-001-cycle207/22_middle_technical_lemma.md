...
```bash
python3 tools/astis.py agent-note 20260613-054827-700501-ASTIS-SALD-001-cycle207 --role middle_technical_lemma --message "..."
python3 tools/astis.py trial-log --task ASTIS-SALD-001 --role middle_technical_lemma --kind handoff --status queued --artifact runs/20260613-054827-700501-ASTIS-SALD-001-cycle207 --notes "..."
```
...
Middle-panel profile: technical-lemma retrieval curator. Focus only on reusable background facts. Search ASTIS-owned declarations, `AutoSamplingTheory/TechnicalLemmas`, `research-wiki/technical-lemmas`, the retrieval index, and local SLT reference material as provenance only. Classify each needed fact as compiled-local, needs-small-port, source-contract-gap, or irrelevant. Do not call upstream SLT as a dependency; every reusable fact must become ASTIS-owned Lean code or an explicit ProofObligation.
