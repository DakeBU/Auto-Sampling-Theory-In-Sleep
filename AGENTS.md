...
Use `.agents/prompts/discrete-sampling.md`, `Libraries/DiscreteSampling/source-map.json` and `route-plan.json`. This is a finite-state source spine, not Euler time discretization. Reuse state-neutral kernels, generator forms and scalar decay; do not build a second probability library or wait for SDE analysis. Every discrete Frontier Cell requires the state/support/pinning, clock/update normalization, kernel/generator, ergodicity, parameter regime, metric, start and cost contract. The existing schema-v3 SAU conceptual-mirror gate also applies. New candidate bridges are preview-only until independent source/mathematical review, and never imply a Lean certificate. The monograph's §12 explicitly omits Ising/coloring proofs; complete them from exact cited originals.
