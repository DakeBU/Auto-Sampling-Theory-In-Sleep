#!/usr/bin/env python3
"""Stage the checked static site for Sites and the legacy worker adapter."""
...
def filesystem_path(path: Path) -> str:
    """Use extended Windows paths for long declaration filenames during staging."""
...
def build_bundle() -> None:
    ...
    for target in (BUNDLE, STATIC):
        if target.is_symlink() or target.resolve().parent != ROOT.resolve():
            raise SystemExit(f"refusing unsafe generated output: {target.name}")
    if STATIC.exists() and not (STATIC / ".astis-generated-static").is_file():
        raise SystemExit("build/ is not an ASTIS-generated static bundle; preserve it")
    shutil.rmtree(filesystem_path(BUNDLE), ignore_errors=True)
    shutil.copytree(filesystem_path(SITE), filesystem_path(BUNDLE / "assets"))
    (BUNDLE / "worker.js").write_text(WORKER, encoding="utf-8", newline="\n")
    shutil.rmtree(filesystem_path(STATIC), ignore_errors=True)
    shutil.copytree(filesystem_path(SITE), filesystem_path(STATIC))
    (STATIC / ".astis-generated-static").write_text("Generated by build_sites_bundle.py\n", encoding="utf-8")
