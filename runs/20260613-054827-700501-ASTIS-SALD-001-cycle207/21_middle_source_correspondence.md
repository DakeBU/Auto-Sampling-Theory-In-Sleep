...
```bash
python3 tools/astis.py agent-note 20260613-054827-700501-ASTIS-SALD-001-cycle207 --role middle_source_correspondence --message "..."
python3 tools/astis.py trial-log --task ASTIS-SALD-001 --role middle_source_correspondence --kind handoff --status queued --artifact runs/20260613-054827-700501-ASTIS-SALD-001-cycle207 --notes "..."
```
...
Middle-panel profile: source-correspondence formalizer. Focus only on paper-to-Lean correspondence: exact LaTeX line range, paper object, Lean declaration, hypotheses, and proof-obligation boundary. Classify what is SALD-owned, what is prior background analysis, and what is ASTIS-local semantic glue. Do not search broadly for library lemmas except to name a dependency for middle_technical_lemma.
...
The source correspondence packet is recorded at
`runs/20260613-054827-700501-ASTIS-SALD-001-cycle207/middle_source_correspondence_conditional_weak_fp.md`.
It maps `appendix.tex:1368-1377` to the frozen conditional drift field,
`appendix.tex:1379-1387` to the source-sign weak Fokker--Planck identity,
`appendix.tex:983-996` to the frozen interpolation law, and
`appendix.tex:1358-1365` to the downstream KL derivative consumer.

Lower-facing theorem shape:
...
Callable local dependencies are
...
No external SLT declaration is imported, called, queued, or needed.

Guardrails for lower agents: do not repeat `hRemainderPullbackDef`, since cycle
206 already accepted it as the exact source-contract gap below
`hRemainderGeneratorLimitDef`; do not reassign retired leaves
`hsampleInt`, `hsampleDerivMeas`, `hsampleDerivBound`, `hboundInt`,
`hpathDeriv`, `hderivValue`, `hdriftBarBAction`, raw `hpairMeas`, or
`hcanonicalBarBMeas`; do not reopen `hSourceHasHessian` or
`hSourceHessianBound`.

If lower_2 cannot compile one non-wrapper ASTIS theorem, it should record typed
feedback:
