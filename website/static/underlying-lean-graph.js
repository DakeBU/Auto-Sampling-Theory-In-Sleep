(() => {
  ...
  function visibleIds() {
    ...
    if (state.query) {
      ...
      selected = new Set([state.focus]); neighbors(state.focus).forEach(id => selected.add(id)); state.expanded.forEach(id => { selected.add(id); neighbors(id).forEach(n => selected.add(n)); });
    } else {
      selected = new Set(all.filter(n => viewKinds[state.view].has(n.kind)).map(n => n.id));
    ...
    if (selected.size > 190) {
      ...
      }).slice(0,190); selected = new Set(keep);
    }
    return selected;
  }
  ...
  function render() {
    ...
    const defs = svgEl("defs"); const marker=svgEl("marker", {id:"ulg-arrow", viewBox:"0 0 10 10", refX:"9", refY:"5", markerWidth:"6", markerHeight:"6", orient:"auto-start-reverse"}); marker.append(svgEl("path", {d:"M 0 0 L 10 5 L 0 10 z"})); defs.append(marker); svg.append(defs);
    ...
    edges.forEach(e => { const a=positions.get(e.source), b=positions.get(e.target); if(!a||!b)return; const path=svgEl("path", {d:`M ${a.x+220} ${a.y+31} C ${a.x+265} ${a.y+31}, ${b.x-45} ${b.y+31}, ${b.x} ${b.y+31}`, "data-relation":e.relation, "marker-end":"url(#ulg-arrow)"}); const title=svgEl("title"); title.textContent=e.relation; path.append(title); edgeLayer.append(path); });
    [...ids].map(id=>nodes.get(id)).sort((a,b)=>(kindOrder[a.kind]-kindOrder[b.kind])||a.label.localeCompare(b.label)).forEach(n => {
      const p=positions.get(n.id), group=svgEl("g", {class:`ulg-node${state.focus===n.id?" selected":""}`, transform:`translate(${p.x} ${p.y})`, tabindex:"0", role:"button", "data-id":n.id});
    ...
    count.textContent=`${ids.size} nodes · ${edges.length} edges`; empty.hidden=ids.size>0; applyTransform();
  }
  ...
  search.addEventListener("input",()=>{state.query=search.value.trim(); state.focus=""; render(); requestAnimationFrame(fit);});
