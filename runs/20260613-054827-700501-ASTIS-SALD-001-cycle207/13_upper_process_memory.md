...
```bash
python3 tools/astis.py agent-note 20260613-054827-700501-ASTIS-SALD-001-cycle207 --role upper_process_memory --message "..."
python3 tools/astis.py trial-log --task ASTIS-SALD-001 --role upper_process_memory --kind handoff --status queued --artifact runs/20260613-054827-700501-ASTIS-SALD-001-cycle207 --notes "..."
```
...
Upper-panel profile: process and memory auditor. Audit whether recent 6h loops repeated wrapper churn, replayed old context, hid unresolved source-contract gaps, or produced Chinese reports that humans cannot use. Check trial summaries, memory digests, TODOs, technical-lemma status, and report entry points. Recommend one harness/prompt/report change only if it directly improves the next proof cycle.
