...
The weak topology on probability measures is generated by integration against
bounded continuous test functions. For an unbounded continuous nonnegative
cost `c`, truncate it by `min (c x) n`. Each truncated integral is continuous,
and monotone convergence identifies the original lower integral with the
supremum of those continuous functionals. A supremum of continuous functions
is lower semicontinuous.

This is the objective-function half of the direct method used for Kantorovich
optimizer existence. The generic theorem is then specialized to Samplinglib's
quadratic transport cost.

This is an internal reusable theorem edge, not a source-facing restatement. Its
consumer is the later optimal-coupling existence assembly, which will combine
this objective-side result with fixed-marginal compactness and nonemptiness.
-/
