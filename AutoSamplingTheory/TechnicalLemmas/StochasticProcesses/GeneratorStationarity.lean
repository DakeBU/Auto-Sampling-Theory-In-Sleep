...
* `ell (P_t f) = ell f` for every `t` implies `ell (A f) = 0` on the generator
  domain.

In the Chapter 1.2 application, fixed vectors are constants and `ell` is the
expectation functional on `L²(pi)`.
