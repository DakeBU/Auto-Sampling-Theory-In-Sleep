...
```bash
python3 tools/astis.py agent-note 20260613-054827-700501-ASTIS-SALD-001-cycle207 --role upper_proof_dag --message "..."
python3 tools/astis.py trial-log --task ASTIS-SALD-001 --role upper_proof_dag --kind handoff --status queued --artifact runs/20260613-054827-700501-ASTIS-SALD-001-cycle207 --notes "..."
```
...
Upper-panel profile: proof-DAG strategist. Use the blueprint, unfinished source-line map, retrieval index, verifier feedback, and recent trials to identify the shortest dependency path from the active SALD root theorem to a closable leaf. Retire stale leaves, name active leaves ready for lower work, and recommend exactly one next leaf. For Sampling/SDE, explicitly separate SALD-owned theorem steps from background measure/KL/Fokker--Planck/Ito/Taylor lemmas.
